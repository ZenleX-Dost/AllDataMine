...
def load_data(dataset):
    ...
    # Try current directory first, then dataset subfolder
    train_file = f'train_{dataset}.txt' if os.path.exists(f'train_{dataset}.txt') else f'dataset/train_{dataset}.txt'
    test_file = f'test_{dataset}.txt' if os.path.exists(f'test_{dataset}.txt') else f'dataset/test_{dataset}.txt'
    rul_file = f'RUL_{dataset}.txt' if os.path.exists(f'RUL_{dataset}.txt') else f'dataset/RUL_{dataset}.txt'
    ...
    true_rul = pd.read_csv(rul_file, header=None, names=['true_rul'])
...
def preprocess_data(train_data, test_data, dataset):
    ...
    if dataset in ['FD002', 'FD004']:
        ...
        kmeans = KMeans(n_clusters=6, random_state=42, n_init=10)  # Added n_init for consistency
...
def train_models(train_data, test_data, true_rul, feature_cols, dataset, subsample, quick_mode, n_estimators, max_depth):
    ...
    # Store model and scaler in session state for RUL prediction tab
...
# Tabs
tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["Data Overview", "EDA", "Model Training", "Results", "Summary", "Decision", "RUL Prediction"])
...
train_data, test_data, scaler = preprocess_data(train_data, test_data, dataset)
...
with tab7:
    st.header("RUL Prediction Tool")
    st.write("Enter engine parameters to predict Remaining Useful Life:")
    ...
    with st.form("prediction_form"):
        ...
        with col1:
            setting1 = st.number_input("Setting 1", value=0.0, format="%.4f")
            setting2 = st.number_input("Setting 2", value=0.0, format="%.4f")
            setting3 = st.number_input("Setting 3", value=0.0, format="%.4f")
            sensor1 = st.number_input("Sensor 1", value=500.0, format="%.2f")
            sensor2 = st.number_input("Sensor 2", value=640.0, format="%.2f")
            sensor3 = st.number_input("Sensor 3", value=1500.0, format="%.2f")
            sensor4 = st.number_input("Sensor 4", value=1400.0, format="%.2f")
            sensor5 = st.number_input("Sensor 5", value=14.0, format="%.2f")
        
        with col2:
            sensor6 = st.number_input("Sensor 6", value=21.0, format="%.2f")
            sensor7 = st.number_input("Sensor 7", value=550.0, format="%.2f")
            sensor8 = st.number_input("Sensor 8", value=2400.0, format="%.2f")
            sensor9 = st.number_input("Sensor 9", value=9000.0, format="%.2f")
            sensor10 = st.number_input("Sensor 10", value=1.3, format="%.2f")
            sensor11 = st.number_input("Sensor 11", value=47.0, format="%.2f")
            sensor12 = st.number_input("Sensor 12", value=520.0, format="%.2f")
            sensor13 = st.number_input("Sensor 13", value=2400.0, format="%.2f")
        
        with col3:
            sensor14 = st.number_input("Sensor 14", value=8000.0, format="%.2f")
            sensor15 = st.number_input("Sensor 15", value=8.4, format="%.2f")
            sensor16 = st.number_input("Sensor 16", value=0.03, format="%.4f")
            sensor17 = st.number_input("Sensor 17", value=390.0, format="%.2f")
            sensor18 = st.number_input("Sensor 18", value=2300.0, format="%.2f")
            sensor19 = st.number_input("Sensor 19", value=100.0, format="%.2f")
            sensor20 = st.number_input("Sensor 20", value=39.0, format="%.2f")
            sensor21 = st.number_input("Sensor 21", value=23.0, format="%.2f")
        
        submit_button = st.form_submit_button("Predict RUL")
        
        if submit_button:
            # Create input array
            input_data = np.array([[setting1, setting2, setting3, 
            ...
            if 'scaler' in st.session_state:
                # Scale only the sensor and setting columns
                scale_cols_indices = list(range(len(input_data[0])))  # All columns need scaling
                input_scaled = st.session_state.scaler.transform(input_data)
                
                # Create feature array with engineered features set to default values
                # This is a simplified approach - in production, you'd need the full feature engineering
                if 'feature_cols' in st.session_state:
                    # Create a dataframe with the scaled input and add missing engineered features
                    input_df = pd.DataFrame(input_scaled, columns=[f'setting_{i}' for i in range(1, 4)] + [f'sensor_{i}' for i in range(1, 22)])
                    input_df['op_condition'] = 0  # Default operating condition
                    
                    # Add missing engineered features with default values (simplified)
                    for col in st.session_state['feature_cols']:
                        if col not in input_df.columns:
                            input_df[col] = 0  # Default value for missing engineered features
                    
                    # Reorder columns to match training
                    input_final = input_df[st.session_state['feature_cols']].values
                else:
            ...
            if 'model' in st.session_state:
                try:
                    predicted_rul = st.session_state.model.predict(input_final)[0]
                    
                    st.success(f"Predicted Remaining Useful Life: **{predicted_rul:.2f} cycles**")
                    
                    # Add interpretation
                    if predicted_rul > 100:
                        st.info("Engine is in good condition")
                    elif predicted_rul > 50:
                        st.warning("Engine needs monitoring")
                    else:
                        st.error("Engine requires immediate attention")
                except Exception as e:
                    st.error(f"Prediction error: {str(e)}. Please ensure the model is trained first.")
            else:
                st.error("Model not trained yet. Please train the model first in the Model Training tab.")

    # Add sample data button
    if st.button("Load Sample Engine Data"):
        st.info("Sample engine parameters loaded with typical values!")